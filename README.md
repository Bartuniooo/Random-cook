![Logo aplikacji Random Cook](images/Random_cook_logo_long.png)

**Random Cook** to prosta, ale praktyczna aplikacja napisana w Pythonie, która rozwiązuje jeden z największych codziennych problemów:  
👉 *„Co dziś zjeść?”*

Aplikacja losuje posiłki z Twojej własnej bazy ulubionych dań, planując jadłospis od **1 dnia do całego tygodnia**, **bez powtórzeń**. Zero myślenia, zero stresu – pełna losowość i porządek w kuchni 😄

---

## 🚀 Funkcjonalności

- 🎲 **Losowanie posiłków** z bazy danych
- 📅 **Planowanie od 1 do 7 dni**
- 🔁 **Brak powtórzeń** w wygenerowanym planie
- ⚖️ **Losowanie ilości porcji**
- 🧠 **Decyzje bez wysiłku** – aplikacja myśli za Ciebie
- 🗂️ **Własna baza ulubionych posiłków**
- 🐍 Napisana w **Pythonie**

---

## 🧩 Jak to działa?

1. Dodajesz swoje ulubione posiłki do bazy danych  
2. Wybierasz, na ile dni chcesz zaplanować jedzenie  
3. Aplikacja losuje:
   - posiłki
   - ilości
   - bez powtórzeń  
4. Otrzymujesz gotowy plan posiłków 🎉

---

## 🛠️ Technologie

- **Python**
- (opcjonalnie – jeśli używasz)
  - SQLite / plik JSON
  - argparse / CLI
  - random

---

## 📦 Instalacja i uruchomienie

```bash
git clone https://github.com/twoj-login/random-cook.git
cd random-cook
python main.py
